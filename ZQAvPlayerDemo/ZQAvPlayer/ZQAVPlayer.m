...
-(instancetype)initWithFrame:(CGRect)frame Adurl:(NSString*)adUrl videoUrl:(NSString*)videoUrl
{
    self = [super initWithFrame:frame];
    
    if (self) {
        _adUrl = adUrl;
        _contentUrl = videoUrl;
        NSLog(@"当前播放的url = %@",_contentUrl);
        [self initUI];
        [self initData];
        [self initAdUI];
...
-(void)initAdUI
{
    ...
    _touchControlView.userInteractionEnabled = false;
...
-(void)initPlayer
{
    if (_contentUrl.length == 0) {
        _contentUrl = @"http://defualt.m3u8";
    }
    ...
    if (_adUrl.length == 0) {
        if (_playerType == player_online) {
            item  = [AVPlayerItem playerItemWithURL:[NSURL URLWithString:_contentUrl]];
        }else if (_playerType == player_local)
        {
            item  = [AVPlayerItem playerItemWithURL:[NSURL URLWithString:_contentUrl]];
        }
    ...
    {
        NSLog(@"当前播放的广告地址是%@",_adUrl);
        item  = [AVPlayerItem playerItemWithURL:[NSURL URLWithString:_adUrl]];
    }
...
-(void)initControlBar
{
    ...
    //    [_controlBar showBarButtons:control_barButton_download,control_barButton_share,control_barButton_praise,control_barButton_fav,control_barButton_lock];
    //    [_controlBar showBarButtons:control_barButton_lock];
...
-(void)changePlayerItem
{
    ...
    //    AVPlayerItem* newItem = [AVPlayerItem playerItemWithURL:testLocalUrl];
...
-(void)changeVideoUrl:(NSString*)_url
{
    ...
    //    _url = @"http://vod.51yaoshi.com/vod1/course2016/ysglyfg/chapter-5/256/ysglyfg-chapter-5-1-1-256_mp4/tzwj_main.m3u8";
...
{
    //    videoUrl = @"http://58.83.195.177/vod1/course2016/zyxzhzhyjn/zydy/256/zyxzhzsyjn-daoyan-1-0-256_mp4/tzwj_video.m3u8";
    AVPlayerItem* newItem = [AVPlayerItem playerItemWithURL:[NSURL URLWithString:videoUrl]];
    if (_playerType == player_local) {
        NSString* url =  [NSString stringWithFormat:@"http://127.0.0.1:12345/%@",[NSURL URLWithString:videoUrl].lastPathComponent];
        newItem  = [AVPlayerItem playerItemWithURL:[NSURL URLWithString:url]];
    }
    [_player replaceCurrentItemWithPlayerItem:newItem];
    [_player play];
    //    [_player setRate:1.5];
...
-(void)topBarCacheBtnClicked
{
    if ([_delegate respondsToSelector:@selector(playerCacheBtnClicked)]) {
        [_delegate performSelector:@selector(playerCacheBtnClicked)];
    }
}
...
-(void)topBarSelectVideo:(id)video
{
    if ([_delegate respondsToSelector:@selector(playerSelectAtVideo:)]) {
        [_delegate performSelector:@selector(playerSelectAtVideo:) withObject:video];
    }
}
...
-(void)controlBarSetPlayerCurrentPlaySecond:(float)second
{
    ...
    //        if (_player.currentItem.status == AVAssetExportSessionStatusWaiting) {
    //            return;
    //        }
...
-(void)changeAd2NormalUrl
{
    _adUrl = @"";
    [self changeVideoUrl:_contentUrl];
    [self ShowTopAndControBar];
    _touchControlView.userInteractionEnabled = true;
}
...
-(void)updateUI
{
    ...
    if (_player.currentItem.status == AVPlayerItemStatusFailed && _adUrl.length !=0) {
        [self changeAd2NormalUrl];
    }
...
-(void)updateScreenOrientation:(UIDeviceOrientation)_orientation byFullScreenBtn:(BOOL)_byBtn
{
    ...
    [UIView animateWithDuration:duration animations:^{
        ...
        //        [self revoleMediaPlayerStyle:MEDIAPLAYER_SCREEN_HORIZONTAL];
    } completion:^(BOOL finished) {
        //        _isScreenOritation = false;
